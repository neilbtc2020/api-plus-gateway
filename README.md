...
为了回馈活跃用户，API Plus 还提供每周使用排行奖励。每周根据使用额度排名，额外发放 GPT Token 奖励。

| 排名 | 奖励 |
| --- | --- |
| 第 1 名 | 按当周使用额度的 `50%` 赠送等值 GPT Token |
| 第 2 名 | 按当周使用额度的 `30%` 赠送等值 GPT Token |
| 第 3 名 | 按当周使用额度的 `20%` 赠送等值 GPT Token |
| 第 4 ~ 10 名 | 按当周使用额度的 `10%` 赠送等值 GPT Token |

活动说明：

- 统计周期：每周一 `00:00` 到周日 `23:59`
- 奖励发放：次周一联系管理领取令牌
- 排名数据以系统实际统计为准
- 奖励额度有效期 `1` 个月
- 活动长期有效，如有调整以官网和群公告为准
